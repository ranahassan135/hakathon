...
const initialState = { name: "", email: "", password: "", confirmPassword: "", role: "donor" };
...
const Register = () => {
  ...
  const handleChange = (e) => setRegisterData((s) => ({ ...s, [e.target.name]: e.target.value }));
  const handleRoleChange = (e) => setRegisterData((s) => ({ ...s, role: e.target.value }));

  const handleSubmit = (e) => {
    e.preventDefault();
    let { name, email, password, confirmPassword, role } = registerData;
    name = name.trim();
    if (name.length < 3) {
    ...
    if (confirmPassword !== password) {
      window.notify("Confirm password does not match", "error");
      return;
    }
    ...
    axios.post(`http://localhost:8000/auth/register`, formData)
      ...
      .catch((error) => {
        console.log(error);
        window.notify("Something went wrong while registering the user", "error");
      })
      .finally(() => {
      ...
      <div className="relative w-full max-w-6xl bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row transform transition-all duration-500 hover:shadow-3xl">
        ...
        <div className="lg:w-1/2 p-6 sm:p-8 lg:p-10 bg-white/95 backdrop-blur-md">
          ...
          <p className="text-gray-600 mb-8 text-center lg:text-left">
            ...
            <Link to="/auth/login" className="text-primary hover:text-emerald-700 font-semibold transition-colors">
              Log in
            </Link>
          </p>
